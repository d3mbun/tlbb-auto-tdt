...
namespace System.Linq.Parallel
{
	internal sealed class GroupByElementSelectorQueryOperatorEnumerator<TSource, TGroupKey, TElement, TOrderKey> : GroupByQueryOperatorEnumerator<TSource, TGroupKey, TElement, TOrderKey>
	{
		...
		{
			HashLookup<Wrapper<TGroupKey>, ListChunk<TElement>> hashLookup = new HashLookup<Wrapper<TGroupKey>, ListChunk<TElement>>(new WrapperEqualityComparer<TGroupKey>(m_keyComparer));
			...
			while (m_source.MoveNext(ref currentElement, ref currentKey))
			{
				...
				value.Add(m_elementSelector(currentElement.First));
			}
			return hashLookup;
		}
	}
