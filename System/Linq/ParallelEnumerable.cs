...
namespace System.Linq
{
	...
	public static class ParallelEnumerable
	{
		...
		public static bool Any<TSource>(this ParallelQuery<TSource> source)
		{
			...
			IEnumerator<TSource> enumerator = ((queryOperator == null) ? source.GetEnumerator() : queryOperator.GetEnumerator(ParallelMergeOptions.FullyBuffered));
			using (enumerator)
			{
				if (enumerator.MoveNext())
				{
		...
		public static Dictionary<TKey, TSource> ToDictionary<TSource, TKey>(this ParallelQuery<TSource> source, Func<TSource, TKey> keySelector, IEqualityComparer<TKey> comparer)
		{
			...
			QueryOperator<TSource> queryOperator = source as QueryOperator<TSource>;
			IEnumerator<TSource> enumerator = ((queryOperator == null) ? source.GetEnumerator() : queryOperator.GetEnumerator(ParallelMergeOptions.FullyBuffered, suppressOrderPreservation: true));
			using (enumerator)
			{
				while (enumerator.MoveNext())
				{
