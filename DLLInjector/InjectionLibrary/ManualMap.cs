namespace _i
{
    ...
    internal class ManualMap : InjectionMethod
    {
        ...
        private static byte[] ExtractManifest(_i.PortableExecutable image)
        {
            byte[] data = null;
        ...
        public override IntPtr Inject(_i.PortableExecutable image, IntPtr hProcess)
        {
            this.ClearErrors();
            try
            {
                return MapModule(Utils.DeepClone<_i.PortableExecutable>(image), hProcess, true);
            }
        ...
        public override IntPtr Inject(string dllPath, IntPtr hProcess)
        {
            ...
            try
            {
                using (_i.PortableExecutable executable = new _i.PortableExecutable(dllPath))
                {
        ...
        public override IntPtr[] InjectAll(_i.PortableExecutable[] images, IntPtr hProcess)
        {
            this.ClearErrors();
            return Array.ConvertAll<_i.PortableExecutable, IntPtr>(images, pe => this.Inject(pe, hProcess));
        }
        ...
        private static bool LoadDependencies(_i.PortableExecutable image, IntPtr hProcess, int processId)
        {
            ...
            if (list.Count > 0)
            {
                ...
                if (data == null)
                {
                    if (string.IsNullOrEmpty(image.FileLocation) || !File.Exists(System.IO.Path.Combine(System.IO.Path.GetDirectoryName(image.FileLocation), System.IO.Path.GetFileName(image.FileLocation) + ".manifest")))
                    {
                    ...
                    str2 = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(image.FileLocation), System.IO.Path.GetFileName(image.FileLocation) + ".manifest");
                }
                else
                {
        ...
        private static IntPtr MapModule(_i.PortableExecutable image, IntPtr hProcess, bool preserveHeaders = false)
        {
            if (hProcess.IsNull() || hProcess.Compare(-1L))
            {
                throw new ArgumentException("Invalid process handle.", "hProcess");
        ...
        private static void MapSections(_i.PortableExecutable image, IntPtr hProcess, IntPtr pModule)
        {
        ...
        private static void PatchImports(_i.PortableExecutable image, IntPtr hProcess, int processId)
        {
            string lpBuffer = string.Empty;
            string str2 = string.Empty;
            foreach (IMAGE_IMPORT_DESCRIPTOR image_import_descriptor in image.EnumImports())
        ...
        private static void PatchRelocations(_i.PortableExecutable image, IntPtr pAlloc)
        {
